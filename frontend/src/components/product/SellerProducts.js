...
export default function SellerProducts() {
  ...
  useState(() => {
    axios
      .get(`http://localhost:8002/product/seller/${sellerID}`)
      .then((res) => {
        setProduct(res.data);
      })
      .catch((err) => {
        ...
        {product.map((productData) => (
          ...
          >
            ...
            <div className="card-body">
              ...
              <Link to={`/oneProduct/${productData._id}`}>
                <button className="btn btn-info btn-sm">See Details</button>{" "}
              </Link>
              {(user1.user.type == "seller" || user1.user.type == "admin") && (
                <Link to={`/editProduct/${productData._id}`}>
