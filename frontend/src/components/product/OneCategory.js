...
export default function OneCategory() {
  ...
  useEffect(
    () => {
      axios
        .get(`http://localhost:8002/product/category/${category}`)
        .then((res) => {
          setCreams(res.data);
        })
        .catch((err) => {
  ...
  function deleteProduct(id) {
    ...
    if (ans) {
      axios
        .delete(`http://localhost:8002/product/delete/${id}`)
        .then(() => {
  ...
  return (
    <div>
      <br></br>
      <h2
        style={{ display: "inline-block", marginLeft: "2%", display: "inline" }}
      >
        All {category}s
      </h2>
      ...
      <div>
          ...
          >
            ...
            <div className="card-body">
              ...
              <p className="card-text" style={{ color: "darkGreen" }}>
                Rs : {creamData.price}.00
              </p>
              <Link to={`/oneProduct/${creamData._id}`}>
                <button className="btn btn-info btn-sm">See Details</button>{" "}
              </Link>
              {user && user.user.type == "admin" && (
                <Link to={`/editProduct/${creamData._id}`}>
              ...
              {user && user.user.type == "admin" && (
                <button
                  ...
                  onClick={() => deleteProduct(creamData._id)}
        ...
        {creams == "" && (
          <div style={{ textAlign: "center", marginTop: "200px" }}>
            <h4>
              Sorry ! Currently No Products are Avaliable Under This Category
            </h4>
          </div>
        )}
      </div>
